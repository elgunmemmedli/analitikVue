<template>
  <div>
    ...
    <button v-if="desktop.length > companiesLoaded.length" @click="loadMore"
            class="btn btn-primary btn-sm d-block ms-auto">View More
    </button>
    <p v-if="desktop.length === companiesLoaded.length" class="text-end"> View all results </p>  </div>  </div>
</template>
...
export default  {
  ...
  created() {
    axios.get('http://ga.loc/api/v1/desktop').then(({data}) =>{
      this.desktop = data;
    })
  },
  methods: {
    loadMore() {
      if (this.length > this.desktop.length) return;
      this.length = this.length + 10;
    },
  },
  computed: {
    companiesLoaded() {
      return this.desktop.slice(0, this.length);
    },
  },
}
</script>
