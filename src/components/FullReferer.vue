...
<div>
  <div class="card-body">
    <div v-for="item in companiesLoaded" class="mb-4 mt-4">
      <div class="d-flex justify-content-between">
        <div class="font-weight-bold mb-1">{{item.referrer}}</div>
  ...
  <button v-if="fullReferrer.length > companiesLoaded.length" @click="loadMore"
          class="btn btn-primary btn-sm d-block ms-auto">View More
  </button>
  <p v-if="fullReferrer.length === companiesLoaded.length" class="text-end"> View all results </p>
</div>
